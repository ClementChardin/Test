...
class Match():
    ...
    def jouer(self,
        ...
        cartes1 = []
        for ii in range(n1):
            cartes1.append(misc.tirer_carte())
        cartes2 = []
        for ii in range(n2):
            cartes2.append(misc.tirer_carte())
        ...
        if sauver:
            ...
            if not prolongation:
                ...
                if self.c_ou_s == 's':
                    ...
                    self.eq1.equipe.sauvegarder()
                    self.eq2.equipe.sauvegarder()
    ...
    def sauver_dicts(self):
        """
        Sert à sauvegarder les points qui ont été marqués et qui sont stockés
        dans les dict_essais, dict_penalites, etc
        """
        for attr in ('essais', 'penalites', 'penalite_ratees', 'drops',
                     'drop_rates', 'transformations', 'transformation_ratees',
                     'jaunes', 'rouges', 'blessures'):
            for eq in self.eq1, self.eq2:
                eqq = eq.equipe
                dd = getattr(eq, 'dict_' + attr)
                for jj in eqq.get_all_joueurs():
                    if jj.nom in dd.keys():
                        val_s = getattr(jj, attr + '_saison')
                        setattr(jj, attr + '_saison', val_s + dd[jj.nom])
                        if not attr == 'drop_rates':
                            val_t = getattr(jj, attr + '_total')
                            setattr(jj, attr + '_total', val_t + dd[jj.nom])
                        else:
                            val_t = getattr(jj, 'drop_ratees_total')
                            setattr(jj, 'drop_ratees_total', val_t + dd[jj.nom])
        self.eq1.equipe.sauvegarder()
        self.eq2.equipe.sauvegarder()
...
def exempter_de_match(nom_ou_noms, c_ou_s='c', clubs=None):
    ...
    if c_ou_s == 'c':
        ...
        for jj in equipe.get_all_joueurs():
            ...
            print cc.nom, cc in clubs_a_sauver
