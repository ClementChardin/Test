...
class Col3Widget(QtGui.QWidget):
    ...
    def setup_model(self):
        ...
        for p in sorted(self.club.joueurs.keys(), key=lambda p:s.ordre_postes[p]):
            for jj in self.club.joueurs[p]:
                self.mod.setItem(r, 0, MyItem(jj.nom, club=self.club))
                for poste in sorted(s.ordre_postes.keys(), key=lambda p:s.ordre_postes[p]):
                    if poste in jj.postes.values() and poste != 'CE':
                        ...
                        self.mod.setItem(r, s.ordre_postes[poste]+1, MyItem(('%0.2f' % s.calc_EV(jj, poste) + num), club=self.club) )#))
                r += 1
    ...
    def func(self):
        ...
        print nom, self.mod.item(r,0).sortKey
...
class BioPopup(QtGui.QWidget):
    ...
    def setup_ui(self):
        ...
        self.nom_lab = QtGui.QLabel(self.joueur.nom)
        self.lay.addWidget(self.nom_lab)
        
        self.form_lay = QtGui.QFormLayout()

        for car in sorted(s.ordre_caracs_joueurs, key=lambda car: s.ordre_caracs_joueurs[car]):
            self.form_lay.addRow(car, QtGui.QLabel(str(self.joueur.caracs[car])))
