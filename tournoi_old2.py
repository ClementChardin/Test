...
class Match():
    ...
    def jouer(self,
        ...
        for ev in evs1:
            if ev == "blessure":
                self.eq1 = jouer_blessure(self.eq1, sauver, clubs=self.clubs, c_ou_s=self.c_ou_s)
        ...
        for ev in evs2:
            if ev == "blessure":
                self.eq2 = jouer_blessure(self.eq2, sauver, clubs=self.clubs, c_ou_s=self.c_ou_s)
            elif ev in ("jaune", "rouge", "jocker"):
        ...
        for tt in tps_forts1:
            self.eq1, self.eq2 = jouer_tps_fort(self.eq1, self.eq2, tt, sauver=sauver, clubs=self.clubs, c_ou_s=self.c_ou_s)
            print "points marques :", self.eq1.equipe.nom
            for attr in ('essais', 'penalites', 'drops', 'transformations'):
                attr = attr + '_saison'
                for jj in self.eq1.equipe.get_all_joueurs():
                    val = getattr(jj, attr)
                    if not val == 0:
                        print jj.nom, attr, val
        for tt in tps_forts2:
            self.eq2, self.eq1 = jouer_tps_fort(self.eq2, self.eq1, tt, sauver=sauver, clubs=self.clubs, c_ou_s=self.c_ou_s)
            self.eq1, self.eq2 = jouer_tps_fort(self.eq1, self.eq2, tt, sauver=sauver, clubs=self.clubs, c_ou_s=self.c_ou_s)
            print "points marques :", self.eq2.nom
            for attr in ('essais', 'penalites', 'drops', 'transformations'):
                attr = attr + '_saison'
                #for eq in (self.eq1, self.eq2):
                for jj in self.eq2.equipe.get_all_joueurs():
                    val = getattr(jj, attr)
                    if not val == 0:
                         print jj.nom, attr, val
        ...
        if phase_finale and self.gagnant is None:
            self.eq1, self.eq2 = self.jouer(sauver, phase_finale=True, prolongation=True)
