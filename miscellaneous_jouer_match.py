...
def tester_M(comp_att, comp_def, bonus):
    ...
    carte_att = tirer_carte()
    carte_def = tirer_carte()
    if carte_att["valeur"] == 14 or carte_def["valeur"] == 1:
        print "Evenement special : essai"
        return True
    elif carte_att["valeur"] == 1 or carte_def["valeur"] == 14:
        print "Evenement special : pas essai"
        return False
    else:
...
def jouer_essai_complet(eq_att,
    ...
    if st == "non":
        ...
        jouer_essai_complet(eq_def, eq_att, diff, 'ar', True, sauver, c_ou_s)
    elif st == "essai_auto":
        ...
        if jouer_tab(buteur, zone, bonus_malus, 'TB'):
            marque("transformation", buteur, eq_att, sauver, clubs, c_ou_s)
        else:
        ...
        if essai: #Test essai reussi
            ...
            jouer_drop(eq_att, zone=1, bonus_malus=0, sauver=sauver, clubs=clubs, c_ou_s=c_ou_s)
...
def jouer_tps_fort(eq_att, eq_def, tps_fort, sauver=True, clubs=None, c_ou_s='c'):
    ...
    if typ in ("melee", "touche"):
        ...
        if tupl[0] == "non":
            ...
            bonus_malus = 0
            if typ == 'touche':
                marque("essai", comp_att.joueurs["n2"], eq_att, sauver, clubs, c_ou_s)
            elif typ == 'melee':
                marque("essai", comp_att.joueurs["n8"], eq_att, sauver, clubs, c_ou_s)
            if jouer_tab(buteur, zone, bonus_malus, 'TB'):
                marque("transformation", buteur, eq_att, sauver, clubs, c_ou_s)
            else:
                marque("transformation_ratee", buteur, eq_att, sauver, clubs, c_ou_s)
            ...
            jouer_essai_complet(eq_att, eq_def, bonus, av_ar, test_M=False, sauver=sauver, clubs=clubs, c_ou_s=c_ou_s)
    ...
    #Si Penalite
        ...
        bonus_malus = 0
        if jouer_tab(buteur, zone, bonus_malus, 'TB'):
            marque("penalite", buteur, eq_att, sauver, clubs, c_ou_s)
        else:
            marque("penalite_ratee", buteur, eq_att, sauver, clubs, c_ou_s)
    ...
    #Si essai
        ...
        jouer_essai_complet(eq_att, eq_def, bonus, av_ar, test_M, sauver,clubs=clubs, c_ou_s=c_ou_s)
...
def attribuer_essai(comp, av_ar, Oplus_M):
    ...
    if n <= fixe*2:
        ...
        nums_idx = 0
        num = nums[nums_idx]
        cumul = joueurs['n'+str(num)].caracs[car_joueur] * d['n'+str(num)]*2
        while cumul < n:
            nums_idx +=1
            num = nums[nums_idx]
            cumul += joueurs['n'+str(num)].caracs[car_joueur] * d['n'+str(num)]*2
...
def marque(typ, joueur, eq, sauver=True, clubs=None, c_ou_s='c'):
    """
    Si c_ou_s == 's' : j'ai tout laissé comme à la saison 11
    Si c_ou_s == 'c' : cette fonction se contente de noter ce qui est marqué
    dans les dict, et une fonction à la fin de match.jouer() sauvegarde
    """
    attr = "dict_" + typ + "s"
    dd = getattr(eq, attr)
...
def jouer_evenement_sauf_blessure(ev,
    ...
    if ev == "jocker":
        ...
        if rand <= 15:
            ...
            num = 15 + rand - 30
        nom = eq.comp.noms_titulaires[num-1] if num <= 15 \
              else eq.comp.noms_remplacants[num-15 -1]
        if nom in eq.dict_jaunes.keys():
            jouer_evenement_sauf_blessure('rouge',
                                          eq,
                                          sauver,
                                          clubs,
                                          c_ou_s,
            ...
            marque("jaune", eq.comp.joueurs["n"+str(num)], eq, sauver, clubs, c_ou_s)
            return -10
    elif ev == "rouge":
        nom = ''
        num = 0
        while nom in eq.dict_rouges.keys() or num == 0:
            num = random_integers(15)
            nom = eq.comp.noms_titulaires[num-1]
        marque("rouge", eq.comp.joueurs["n"+str(num)], eq, sauver, clubs, c_ou_s)
        if prolongation:
            res = d3_plus()*-10
        else:
            res = d6_plus()*-10
...
def jouer_blessure(eq, sauver=True, clubs=None, c_ou_s='c'):
    ...
    marque("blessure", jj, eq, sauver, clubs, c_ou_s)
