...
class saison(object):
    ...
    def init_quarts(self):
        for nom_championat in ('coupe', 'challenge'):
            classement = self.get_classement(nom_championat)
            cal_name = nom_championat + '_quarts'
            filename = SAISON_DIR_NAME(self.dat) + '/calendrier_' + \
                    cal_name + '.txt'
            ...
            self.quarts_inities = True
            self.sauvegarder()

    def init_demies(self):
        for nom_championat in ('coupe', 'challenge'):
            ...
            self.calendriers.append(cal)
            self.dict_indice_journees[cal_name] = 0
            self.demies_initiees = True
            self.sauvegarder()

    def init_finales(self):
        for nom_championat in ('coupe', 'challenge'):
